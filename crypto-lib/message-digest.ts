...
export namespace md {
    ...
    export let messageDigestType: IMessageDigestTypeStatic = {
    ...
    export function digest(messageDigest: IMessageDigestType, data: Buffer): Buffer {
        ...
        let word = cryptojs.enc.Hex.parse(data.toString("hex"));
        let hash: any;
        ...
        if (messageDigest === messageDigestType.MD5) {
            ...
            let md = forge.sha512.sha224.create();
            ...
            let md = forge.sha512.sha256.create();
            md.update(forgeutil.bufferToString(data));
            return new Buffer(md.digest().toHex(), "hex");
