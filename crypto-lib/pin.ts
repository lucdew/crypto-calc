...
export var isoPinType: IIsoPinTypeStatic = {
    format0: {
...
class IsoPin implements IIsoPin {
    ...
    static parseIsoPinBlock(block: Buffer, pan ? : string): IIsoPin {
        ...
        if (type.value === 0 || type.value === 3) {
            if (!pan) {
                var e: any = new error.CryptoError(error.PAN_MISSING, 'Pan is missing');
                e.additionalInfo = {
                    pinLength: pinLength,
                    pinType: type
                };
...
export function createIsoPin(isoPinType: IIsoPinType, pin: string, additionalData?: string): IIsoPin {
