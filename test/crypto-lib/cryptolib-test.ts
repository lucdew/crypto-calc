    ...
    function assertPad(paddingName:string, testOpts, testName: string) {
        ...
        return () => {
            if (testOpts.expectedError) {
                assertCryptoError(pad.bind(this, testOpts.data, testOpts.size ? testOpts.size : 8), ( < any > error)[testOpts.expectedError]);
            } else {
                if (testOpts.expectedRegex) {
                    assert.match(pad(b(testOpts.data), testOpts.size ? testOpts.size : 8, testOpts.optional).toString('hex'), new RegExp(testOpts.expectedRegex));
                } else {
                    assert.equal(pad(b(testOpts.data), testOpts.size ? testOpts.size : 8, testOpts.optional).toString('hex'), testOpts.expected);
                }
            }
        };
    }
        ...
        if (operation === 'cipher') {
            ...
            it('decipher ' + testName, () => {
                ...
                if (test.expectedError) {
                    assertCryptoError(cipher.decipher.bind(this, key, data, cipherAlgo, cipherMode, cipherOpts), ( < any > error)[test.expectedError]);
                } else {
