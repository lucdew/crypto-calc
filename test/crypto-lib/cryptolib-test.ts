...
describe('Padding', () => {
    ...
    for (var aPadding in fixtures.padding) {
	    ...
	    function assertPad(padding:string,testName:string) {
	    ...
	    function assertUnpad(padding:string,testName:string) {
			var unpad = (<any>cryptolib.padding)[padding].unpad;
			return () => {
	...
	function createCipherDecipherTest(operation:string,cipherAlgoName:string,testName:string,test:any) {
		...
		if (operation==='cipher') {
				   ...
				   it('decipher '+testName,()=> {
						...
						var result:Cryptolib.Cipher.ICipherResult = null;
						if (test.expectedError) {
							 assertCryptoError(cipher.decipher.bind(this, key,data,cipherAlgo,cipherMode,cipherOpts),
								  (<any>cryptolib.error)[test.expectedError]);
						}
						else {
